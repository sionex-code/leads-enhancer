// Opportunity scoring, in CommonJS so the server can run the exact same code
// the browser does.
//
// This moved out of app/lib/opportunity.js when the leads table gained an
// "Best opportunity" sort. That table pages server-side, so the ordering has to
// be decided in the API - and a second implementation of these weights, in SQL
// or anywhere else, would drift from the number printed in the column. One
// module, imported by both sides, is the only way the sort and the score can be
// guaranteed to agree.
//
// app/lib/opportunity.js re-exports this for client components.
...
const trackingDetect = require("./tracking-detect.cjs");
